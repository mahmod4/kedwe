...
async function fetchUserProfile(userId) {
  try {
    // استعلام متقدم لجلب بيانات محددة من ملف تعريف المستخدم
    const { data, error } = await supabase
      .from('profiles')
      .select('user_id, full_name, email, avatar_url, role, created_at')
      .eq('user_id', userId)
      .single();
    ...
    console.log('تم جلب ملف تعريف المستخدم:', data);
    return data;
  } catch (error) {
